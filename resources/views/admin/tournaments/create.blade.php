...
<div class="chess-form">
    <h1 style="color:#8b4513;text-align:center;margin-bottom:30px;">Создание нового турнира</h1>
    ...
    <form action="{{ route('admin.tournaments.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <select name="gamemode_id" class="form-control @error('gamemode_id') is-invalid @enderror" required>
                ...
                @foreach($gamemodes as $gamemode)
                    <option value="{{ $gamemode->id }}" {{ old('gamemode_id') == $gamemode->id ? 'selected' : '' }}>
                        {{ $gamemode->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <input type="number" name="quantity_rounds" class="form-control @error('quantity_rounds') is-invalid @enderror" 
                   value="{{ old('quantity_rounds', 1) }}" min="1" required>
        ...
        <div class="form-group">
            ...
            <select name="status" class="form-control @error('status') is-invalid @enderror" required>
                <option value="Запланирован" {{ old('status') == 'Запланирован' ? 'selected' : '' }}>Запланирован</option>
                <option value="В процессе" {{ old('status') == 'В процессе' ? 'selected' : '' }}>В процессе</option>
                <option value="Завершен" {{ old('status') == 'Завершен' ? 'selected' : '' }}>Завершен</option>
            </select>
            @error('status')
                <div class="text-danger">{{ $message }}</div>
            @enderror
